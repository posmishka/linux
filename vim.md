...
`:5,.m$
` поместить в конец файла блок строк с 5й по текущую
`:%d` удалить все строки в файле
`:%t$
` скопировать все строки и поместить в конец файла
`:.,.+20d` удалить текущую строку и 20 строк после неё
`:10,$
m.-2` строки с десятой по конец файла переместить на две строки выше текущей
...
`:100,$
w newfile` сохраняет от 100 строки до конца файла в новый файл. См. синтаксис ed. Файл перезапишется.
`:340,$
w >>newfile` дописать строки с 340 до конца в новый файл
...
`:1,$
s/old/new/g` то же что `:%s/old/new/g` замена по всему файлу
